...
- 🎨 **嵌入式樹狀圖編輯器** (2025-10-09)
  - 創建 EmbeddedTreeDiagram 組件，專為專案頁面設計
  - 移除左側導航區域（80px）和上方工具列
  - 保留樹狀圖主編輯區和右側屬性面板（320px，可摺疊）
  - 完整的鍵盤快捷鍵支援：Tab 新增節點、Delete 刪除節點、Esc 取消
  - 右鍵選單支援新增和刪除節點
  - 雙擊節點或在屬性面板編輯節點名稱
  - 根節點保護機制（無法刪除）
  - 程式碼減少 69%（1228 行 → 380 行）
  - 更大的編輯區域，更適合嵌入專案頁面

### 變更（Changed）

- 🎨 **專案頁面整合完整樹狀圖編輯器** (2025-10-09)
  - 將完整的 TreeDiagram 組件（包含左側工具列、上方工具欄、右側屬性面板）整合到專案主視窗黃色編輯區域
  - 保留所有原始功能：工具導覽、節點編輯、屬性面板、快捷鍵等
  - 調整 CSS 讓 TreeDiagram 適應父容器高度（height: 100vh → 100%）
  - 完整的 1:8:1 三欄布局嵌入專案頁面
  - 移除 SimpleTreeDiagram 簡化版本

- 🎨 **專案樹狀圖簡化** (2025-10-09)
  - 建立 SimpleTreeDiagram 組件，只保留主編輯區域
  - 移除左側導航工具列
  - 移除上方工具欄
  - 移除右側屬性面板
  - 保留核心樹狀圖顯示和互動功能
  - 減少組件複雜度（從 1228 行簡化到 176 行）
  - 提升載入速度和記憶體效能

### 新增（Added）

- 🌳 **專案樹狀圖整合** (2025-01-15)
  - 將樹狀圖編輯器整合到專案主視窗黃色編輯區域
  - Projects 表新增 4 個樹狀圖欄位：`tree_data`（JSONB）、`tree_config`（JSONB）、`tree_version`（INTEGER）、`tree_updated_at`（TIMESTAMP）
  - 新增 GIN 索引用於高效 JSONB 查詢
  - 實現樹狀圖自動載入與儲存機制
  - 新增 API 端點：`PUT /api/projects/:id/tree`
  - 自動版本號遞增和時間戳記更新
  - 新增儲存狀態指示器
  - 無樹狀圖時顯示建立提示
...
- 🎨 **專案主視窗導航區調整** (2025-10-09)
  - 清空左側紅色導覽區按鈕,保留容器結構
  - 移除暫時性的五個導航按鈕 (O/S/D/V/T)
  - 為後續自訂導航功能預留空間

### 新增（Added）

- 🎨 **專案主視窗完整實作** (2025-10-09)
  - 根據設計圖實作專案主視窗四色布局
  - 標題列（Header）：藍色漸層背景，64px 高度，顯示專案資訊和操作按鈕
  - 窄導航欄（Navi）：紅色漸層背景，60px 寬度，圖標式導航（總覽/規格/設計/開發/測試）
  - 中間預覽欄（Preview）：綠色漸層背景，280px 寬度，顯示專案詳細導航選單
  - 主要編輯區（Editor）：黃色漸層背景，彈性寬度（1fr），顯示動態內容
  - 新增 ProjectMainWindow 元件（131 行）
  - 新增 ProjectMainWindow.css 完整樣式定義
  - 新增 ProjectPage 入口頁面
  - 從首頁點擊專案卡片在新視窗開啟專案主視窗
  - 支援動態內容切換和響應式設計（< 768px 隱藏中間預覽欄）
...
- 🔧 **首頁專案卡片優化** (2025-10-09)
  ...
  - 移除副標題顯示，優化視覺密度
  - 專案統計區域移至專案列表上方
  - 點擊專案卡片使用 window.open 開啟新視窗（1400x900px）
  - 新視窗 URL 格式：`/?view=project-page&id={projectId}`

- 📊 **首頁專案展示功能** (2025-10-09)
  - 首頁初始化時自動檢查 projects 資料表是否存在
  - 顯示最新編輯的 5 個專案（按 updated_at DESC 排序）
  - 專案卡片資訊：專案名稱、中英文副標題、遊戲類型標籤、專案狀態徽章、最後編輯時間
  - 專案統計：總專案數、資料庫連線狀態
  - 視覺設計：漸層黃色卡片、左側橙色高亮條、Hover 向右平移效果
  - 空狀態提示：無專案時引導建立第一個專案
  - 完整專案管理功能保留在「專案管理視窗」

- 🗂️ **Projects 資料表結構升級** (2025-10-09)
  - 新增 `name` 欄位作為主要顯示名稱
  - 將 `name_ch` 重命名為 `name_zh` 統一命名規範
  - 自動資料庫遷移：檢測並重新命名欄位、回填資料、建立索引
  - TypeScript 型別同步更新

- 🚀 **Projects API 增強** (2025-10-09)
  - GET /api/projects 支援 limit、offset、orderBy 查詢參數
  - 首頁使用優化查詢：`/api/projects?limit=5&orderBy=updated_at DESC`

- 🌳 **樹狀圖展開/收合按鈕系統** (2025-10-09)
  - 每個有子節點的節點右側顯示圓形 +/- 按鈕
  - 視覺設計：
    - 28×28px 圓形按鈕
    - 半透明深色背景（rgba(15, 23, 42, 0.6)）
    - Hover 效果：藍紫色高亮（rgba(99, 102, 241, 0.8)）
    - 按鈕位置：right: -26px, top: calc(50% - 1px), translateY(-75%)
  - 互動行為：
    - 點擊節點：僅選取節點，開啟屬性面板
    - 點擊按鈕：展開/收合子節點
    - 展開顯示 "−"，收合顯示 "+"
  - 技術實現：
    - 使用 `hasChildren` 標記判斷是否顯示按鈕
    - `toggleNodeCollapse` 獨立控制收合狀態
    - `overflow: visible` 確保按鈕不被節點裁切
    - `stopPropagation` 避免按鈕點擊觸發節點選取

- 🗂️ **樹狀圖資料持久化系統增強** (2025-10-09)
  - 新增/刪除節點後自動儲存到 localStorage
  - 改進合併邏輯：
    - 保留原始結構中的節點
    - 合併已編輯節點的 metadata
    - **保留新增節點**：從 savedTree 複製原始結構沒有的節點
    - 遞迴合併 children，確保完整樹狀結構
  - 功能完整性：
    - ✅ 新增節點 → 儲存 → 重新載入後保留
    - ✅ 刪除節點 → 儲存 → 重新載入後移除
    - ✅ 編輯 metadata → 儲存 → 重新載入後保留
    - ✅ 展開/收合狀態獨立管理

- ⌨️ **樹狀圖鍵盤快捷鍵** (2025-10-09)
  - Tab：新增子節點（選取節點後按 Tab）
  - Delete/Backspace：刪除節點（選取節點後按刪除鍵，根節點除外）
  - 安全防護：編輯模式下鍵盤快捷鍵自動停用
  - 刪除保護：無法刪除根節點
