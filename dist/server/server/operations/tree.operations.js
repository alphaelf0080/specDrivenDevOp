...
export class TreeOperations {
    ...
    calculateNodeCount(node) {
        ...
        if (node.children && node.children.length > 0) {
            count += node.children.reduce((sum, child) => sum + this.calculateNodeCount(child), 0);
        }
        return count;
    }
    /**
     ...
     * 根據 ID 取得樹狀圖
     */
    async getTreeById(id, includeDeleted = false) {
        const client = await this.pool.connect();
        try {
            const query = `
        SELECT * FROM trees
     ...
     * 根據 UUID 取得樹狀圖
     */
    async getTreeByUuid(uuid, includeDeleted = false) {
        const client = await this.pool.connect();
        try {
            const query = `
        SELECT * FROM trees
     ...
     * 取得專案的所有樹狀圖
     */
    async getTreesByProject(projectId) {
 ...
 * 建立樹狀圖操作實例的工廠函數
 */
export function createTreeOperations(pool) {
