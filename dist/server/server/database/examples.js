...
export async function exampleUpdate() {
    ...
    await db.connect();
    try {
        const updatedUsers = await db.update('users', { email: 'newemail@example.com', updated_at: new Date() }, { username: 'john_doe' });
...
export async function exampleDelete() {
    ...
    await db.connect();
    try {
        const deletedCount = await db.delete('users', { username: 'john_doe' });
...
export async function exampleTransaction() {
    ...
    try {
        const result = await db.transaction(async (client) => {
            ...
            await client.query('INSERT INTO transactions (account_id, amount, type) VALUES ($1, $2, $3)', [1, 1000, 'deposit']);
            return { success: true };
        });
