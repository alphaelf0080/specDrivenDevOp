 ...
 */
import { ColumnType, tableConfigs, getTableConfig, } from '../config/table.config.js';
/**
 * 生成欄位的 SQL 定義
 */
 ...
 */
export function generateCreateIndexSQL(tableName, index) {
    const table = getTableConfig(tableName);
    if (!table) {
        throw new Error(`找不到資料表配置: ${tableName}`);
    }
 ...
 * 生成所有索引的 SQL
 */
export function generateAllIndexesSQL(tableName) {
    const table = getTableConfig(tableName);
 ...
 * 生成完整的資料表建立 SQL（包含索引）
 */
export function generateFullTableSQL(tableName) {
 ...
 * 生成所有資料表的建立 SQL
 */
export function generateAllTablesSQL() {
 ...
 * 生成 SELECT 查詢語句
 */
export function generateSelectSQL(tableName, options) {
    const table = getTableConfig(tableName);
    if (!table) {
        throw new Error(`找不到資料表配置: ${tableName}`);
    }
 ...
 * 生成 INSERT 語句
 */
export function generateInsertSQL(tableName, data, returning = true) {
    const table = getTableConfig(tableName);
    if (!table) {
        throw new Error(`找不到資料表配置: ${tableName}`);
    }
 ...
 * 生成 UPDATE 語句
 */
export function generateUpdateSQL(tableName, data, where) {
    const table = getTableConfig(tableName);
    if (!table) {
        throw new Error(`找不到資料表配置: ${tableName}`);
    }
 ...
 * 生成 DELETE 語句
 */
export function generateDeleteSQL(tableName, where) {
    const table = getTableConfig(tableName);
    if (!table) {
        throw new Error(`找不到資料表配置: ${tableName}`);
    }
 ...
 * 生成 DROP TABLE 語句
 */
export function generateDropTableSQL(tableName, cascade = false) {
    const table = getTableConfig(tableName);
    if (!table) {
        throw new Error(`找不到資料表配置: ${tableName}`);
    }
 ...
 * 驗證資料是否符合資料表定義
 */
export function validateData(tableName, data) {
    const table = getTableConfig(tableName);
    if (!table) {
