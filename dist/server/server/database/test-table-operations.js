...
async function testTableOperations() {
    ...
    try {
        ...
        // 4. 查詢單一資料
        console.log('\n4️⃣  查詢單一資料...');
        const singleItem = await db.queryOne('SELECT * FROM test WHERE id = $1', [newItem.id]);
        if (singleItem) {
        ...
        // 5. 更新資料
        console.log('\n5️⃣  更新資料...');
        const updatedItems = await db.update('test', {
            status: 'completed',
        ...
        // 6. 使用 WHERE 條件查詢
        console.log('\n6️⃣  查詢 active 狀態的項目...');
        const activeItems = await db.queryMany('SELECT * FROM test WHERE status = $1 ORDER BY created_at DESC', ['active']);
        console.log(`   找到 ${activeItems.length} 筆 active 項目`);
        // 7. 使用 JSONB 查詢
        console.log('\n7️⃣  使用 JSONB 查詢 (priority = high)...');
        const highPriorityItems = await db.queryMany("SELECT * FROM test WHERE data->>'priority' = $1", ['high']);
        console.log(`   找到 ${highPriorityItems.length} 筆高優先級項目`);
        highPriorityItems.forEach((row) => {
    ...
    `);
        ...
        const transactionResult = await db.transaction(async (client) => {
            const item1 = await client.query('INSERT INTO test (name, description, status) VALUES ($1, $2, $3) RETURNING *', ['交易測試 1', '第一筆交易資料', 'active']);
            const item2 = await client.query('INSERT INTO test (name, description, status) VALUES ($1, $2, $3) RETURNING *', ['交易測試 2', '第二筆交易資料', 'active']);
            return {
                count: 2,
...
//# sourceMappingURL=test-table-operations.js.map