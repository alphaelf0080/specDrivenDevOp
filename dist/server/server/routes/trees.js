 ...
 */
router.get('/project/:projectId/main', async (req, res) => {
    try {
        const { projectId } = req.params;
        // 先查詢專案的 main_tree_id
        const projectResult = await db.query('SELECT main_tree_id FROM projects WHERE id = $1 AND deleted_at IS NULL', [projectId]);
...
router.post('/', async (req, res) => {
    try {
        ...
        if (treeData.projectId && req.body.setAsMain) {
            await db.query('UPDATE projects SET main_tree_id = $1 WHERE id = $2', [newTree.id, treeData.projectId]);
        }
