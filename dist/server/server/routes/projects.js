...
router.post('/', async (req, res) => {
    try {
        const { name, name_zh, name_en, game_type, description, status = 'active', owner_id, } = req.body;
        // 驗證必填欄位
      ...
      RETURNING *`, [name, name_zh, name_en, game_type, description || null, status, owner_id || null]);
...
router.put('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const { name, name_zh, name_en, game_type, description, status, owner_id, } = req.body;
        // 檢查專案是否存在
