# PowerShell 腳本：將 Gemini SDD 工作流程匯入資料庫

Write-Host "🚀 開始匯入樹狀圖到資料庫...`n" -ForegroundColor Cyan
...
try {
    ...
    Write-Host "✅ 樹狀圖匯入成功!`n" -ForegroundColor Green
    Write-Host "📊 匯入結果:" -ForegroundColor Yellow
    Write-Host "  - ID: $($result.id)"
    Write-Host "  - UUID: $($result.uuid)"
    Write-Host "  - 名稱: $($result.name)"
    Write-Host "  - 搜尋鍵: $($result.search_key)"
    Write-Host "  - 節點數: $($result.node_count)"
    Write-Host "  - 深度: $($result.depth)"
    Write-Host "  - 標籤: $($result.tags -join ', ')"
    Write-Host "`n🌐 在首頁即可查看此樹狀圖!" -ForegroundColor Cyan
}
catch {
